...
const equipment = [
  ...
  { name: '力量戒指', slot: 'accessory', stats_bonus: JSON.stringify({ attack: 8, defense: 3, speed: 2 }), price: 250, rarity: 'epic', required_level: 15, image_url: '/images/equipment/力量戒指_ai.png' },
  { name: '精灵护符', slot: 'accessory', stats_bonus: JSON.stringify({ attack: 3, defense: 3, speed: 15 }), price: 400, rarity: 'rare', required_level: 20, image_url: '/images/equipment/精灵护符_ai.png' },
...
async function createRichTestData() {
  ...
  const initCrypto = require('crypto');
  const initUsedSlugs = new Set(db.prepare('SELECT slug FROM classes WHERE slug IS NOT NULL').all().map(r => r.slug));
  function initGenerateSlug(name) {
    const base = name.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, '-').replace(/^-+|-+$/g, '');
    let slug;
    do {
      const suffix = initCrypto.randomBytes(3).toString('hex');
      slug = `${base || 'class'}-${suffix}`;
    } while (initUsedSlugs.has(slug));
    initUsedSlugs.add(slug);
    return slug;
  }
  const insertClass = db.prepare('INSERT INTO classes (name, grade, slug, school_id, student_count) VALUES (?, ?, ?, ?, ?)');
  const classIds = [];
  classes.forEach(cls => {
    const slug = initGenerateSlug(cls.name);
    const result = insertClass.run(cls.name, cls.grade, slug, schoolId, 0);
